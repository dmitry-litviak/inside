	...
	public function action_index()
	{
            ...
            foreach ($vacations as $key => $vacation) {
                if (Helper_Main::dateDiffInDays(date('d'), date('m'), $vacation->day_start, $vacation->month_start, $vacation->year) == 14) {
                    ...
                    Library_Mail::factory()->setFrom(array('0' => 'noreply@inside.org'))->setTo(array('0' => $vacation->user->email))->setSubject('Отпуск')->setView('mail/vacation', $data)->send();
                    foreach ($mails as $mail) {
                        Library_Mail::factory()->setFrom(array('0' => 'noreply@inside.org'))->setTo(array('0' => $mail->email))->setSubject('Отпуск')->setView('mail/vacation', $data)->send();
                    }
                }
            }
	}
